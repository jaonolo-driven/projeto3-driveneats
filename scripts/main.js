...
for(let i = 0; i < contentSource.length; i++){
    ...
    for(let j = 0; j < contentSource[i].values.length; j++){
        ...
        button.querySelector('button > span').innerText = contentSource[i].values[j].name
        button.querySelector('.description span').innerText = contentSource[i].values[j].description
...
submitButton.addEventListener('click', () => {
    ...
    for(let i = 0; i < selectedButtons.length; i++){
        let foodName = selectedButtons[i].querySelector('button > span').innerText
